...
def predict_image(image_tensor, local_model:bool):
    ...
    if local_model is True:
        ...
        ### FIXME Implement the API pipeline to replace the local model inference
        ### FIXME Use https://huggingface.co/dphn/Dolphin3.0-Qwen2.5-0.5B
        headers = {"Authorization": f"Bearer {HF_API_KEY}"}
        response = requests.post(HF_API_URL, headers=headers, json={"inputs": image_tensor.tolist()})
        if response.status_code == 200:
            result = response.json()
            return result["predicted_class"]
        else:
            print("Error occurred while calling Hugging Face API.")
            return None
...
iface = gr.Interface(
    ...
    ### FIXME add a button for using local model or HF API
    ...
    ### FIXME add examples of FEN inputs
